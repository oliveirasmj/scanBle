...
class ServicesResource(resource.Resource):
    ...
    def __init__(self):
        super().__init__()
        self.allowed_methods = ["GET", "POST", "GET_BY_ID", "DELETE"]
    ...
    async def render_get_by_id(self, request, service_id):
        """
        Handle GET requests to retrieve a service by ID
        """
        service = mongo_collection.find_one({"_id": ObjectId(service_id)})
        if service:
            response = str(service)
        else:
            response = f"Service with ID {service_id} not found"
        return aiocoap.Message(payload=response.encode())
